...
- [x] **Current snow overlays** (2026-08-19) — a *Snow* group in the overlays pane.
  - **Snow depth** and **snow water equivalent** from NOAA **NOHRSC National Snow Analysis** (layers 3 and 7). These are modeled current conditions in inches for CONUS at about 1 km resolution, refreshed four times daily. The M2 route keeps a **two-hour** warm cache and a two-value layer whitelist; the browser now has an authoritative direct-CORS fallback, so losing the private server no longer disables snowpack.
...
- [x] **Geologic and Ice Age reference overlays** (expanded 2026-08-24) — authoritative regional references in the overlays pane.
  ...
  - **WGS DS-18 map units**, **WGS Quaternary active faults**, **WGS volcanic vents**, and **WGS 1:250k continental ice limits** use their exact published MapServer layers browser-direct. The visible notes preserve their regional-scale/site-hazard limitations.
  - **WGS-hosted Ice Age flood affected area** and the **EWU/IAFI Pleistocene-lake reconstruction** load simplified GeoJSON once per session and are explicitly labeled as references/reconstructions, not depth, age, frequency, or present-water observations.
...
- [x] **USGS S3 1 m DEMs as a terrain source** (2026-08-19) — terrain is now computed here from real elevation instead of scraping someone's rendered PNG. New `cog.js` (format) and `usgs.js` (data + rendering); still **zero npm dependencies**, which took writing a TIFF LZW decoder and the floating-point predictor by hand since Node's zlib has neither.
  ...
  - **`POST /api/usgs/warm`** + `/status`, `/stop`, `/areas`, `/areas/check`, `/plan` — resumable area download. Resume needs no bookkeeping: rendered tiles and the byte ranges behind them are both cached, so a restart counts them as skipped. Measured: 84 tiles → 68 fetched / 16 skipped, then a re-run → **0 fetched / 84 skipped**.
  - **Staleness by ETag**, so the catalogue-diff hack is unnecessary for this source. Both diff paths exercised against live S3: unchanged → `stale:false`; tampered baseline → `stale:true` with the live ETag surfaced.
  - **Source selector** gains *USGS 1 m (offline-capable)*. `Best available` still prefers WA DNR where it has data — 345 projects to USGS's 25, some finer than 1 m — then USGS 1 m, then 3DEP, with 3DEP always underneath so a project edge is never a hole.
  - **Two corrections to the research** (`lidar-research.md` Addendum 3), both from measurement. The cell rule is `y = ceil(N/10000)` — the north edge — not `floor()`; every coverage claim in Addendum 2 was checked one 10 km cell too far south. Consequently **Mount Rainier NP has no federal 1 m DEM** (coverage stops hard at `y518`), and **Hurricane Ridge does** have it. Statewide is 2,548 tiles / 474 GiB, not 2,797 / 519 GiB.
  - Bugs found and fixed while building: `NODATA = -1e30` compared against a `Float32Array` never matches (needs `Math.fround`), which silently blanked every pixel; picking an overview *coarser* than the request produced a cross-hatch once the hillshade took gradients of the upsampled steps; a fully-resumed download recorded no sources and wiped its own ETag baseline; and `+null === 0` let `/api/usgs/cover` answer for the Gulf of Guinea when called with no parameters.
  - Verified: Mt Baker summit reads 3282.6 m from an 8 m overview vs 3286 m published; a warmed tile re-renders **byte-identically with zero network calls attempted**; all styles 200/204 with no 5xx; `node --check` clean on all three files; no `npm install`.
  - *Acceptance deviation:* the spec asked for "Rainier at z16 renders hillshade from a locally cached DEM tile". Rainier has no federal 1 m coverage, so that test is not achievable as written — it was run at **Mt Baker** instead, which also exercises the older filename convention.
...
- [x] **Explicit date ranges, and one date path for every source** (2026-08-19) — the time window was a `days` integer counted back from *now*, threaded separately into `stacSearch`, `gibsItems` and `stacBbox`, so "imagery from July 2025" was not expressible at all.
  ...
  - **GIBS was the interesting case.** Its near-real-time layers are generated client-side, one tile set per day, previously always counted back from `Date.now()` — so a historical range would have returned today's imagery, silently out of range. It now anchors on the *end* of a historical window, while a current search starts on the last completed UTC day so today's still-partial global mosaic cannot win auto-selection and fail as “too incomplete.” A July 2025 search still yields 2025-07-29/30/31, with `/2025-07-31/` in the tile URL.
